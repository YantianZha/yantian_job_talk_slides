(function(window) {
  ...
  var markerBoxWidth = 80;
  var markerBoxHeight = 80;
  var width;
  var height;
  var tree;
  var svg;
...
function appendArrowheadMarker(svg) {
    svg.append("defs").append("marker")
        ...
        .attr("markerWidth", 10)
        .attr("markerHeight", 7)
        .attr("refX", 65) // Adjust refX to position arrowhead correctly
        .attr("refY", 3.5)
        .attr("orient", "auto")
      .append("polygon")
        .attr("points", "0 0, 10 3.5, 0 7")
        .attr('stroke', '#f6a821')
        .style("fill", '#f6a821'); // Set the fill color of the arrow;
}
  ...
  // Public methods
  function addItem(name, parent, root, radius, color) {
  if (!parent) {
  ...
  function update(data) {
    ...
    .attr("refX", function(d) {
        // Calculate refX based on the node radius and any necessary adjustment
        return d.target.radius + 10; // Adjust as needed
    });
