(function (window) {
  ...
  function renderNodes(nodes, data) {
    ...
    var nodeEnter = node.enter().append("g")
      .attr({
        ...
        transform: function(d) {
          var parentX = d.parent !== undefined ? d.parent.y0 : data.y0;
          var parentY = d.parent !== undefined ? d.parent.x0 : data.x0;
          var x = parentX + (d.relativeX || 0) * width;
          var y = parentY + (d.relativeY || 0) * height;
    ...
    var nodeExit = node.exit().transition()
      .duration(duration)
      .attr("transform", function(d) {
        var parentX = d.parent !== undefined ? d.parent.y : data.y0;
        var parentY = d.parent !== undefined ? d.parent.x : data.x0;
        var x = parentX + (d.relativeX || 0) * width;
        var y = parentY + (d.relativeY || 0) * height;
        return "translate(" + x + "," + y + ")";
      })
  ...
  // Public methods
  function addItem(name, parent, root, relativeX, relativeY, radius, color) {
    if (!parent) {
    ...
    return function() {
      ...
      parentNode.children.push({
        name: name,
        radius: 1, // Include the radius here
        color: color, // Include the color here
        relativeX: relativeX, // Include the relative X coordinate here
        relativeY: relativeY // Include the relative Y coordinate here
      });
      update(root);

      // Transition to final size for the last added node
      var newNodes = svg.selectAll("circle").filter(function(d) { return d.name === name; });
      newNodes.transition()
        .duration(duration)
        .attr('r', radius);
    };
